...
This repository uses a "dehydrated" skill structure to avoid maintaining duplicate files. Skills in `.claude/skills/` contain `-replace.txt` files which point to the "Source of Truth" in the root directory.
...
1. **Scan**: Look for all files ending in `-replace.txt` within the `.claude/skills/` directory.
